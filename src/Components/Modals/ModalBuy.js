...
const ModalBuy = ({ ui, toggleBuyModal, toggleSellModal, socket, stocks, brokers}) => {
    ...
    const buy = (e) => {
        ...
        if (ui.get('isModalBuyOpen') && value * price > brokers.get(0).budget){
        ...
        closeModal();
        socket.socket.emit('offer', transaction);
    }
...
const mapStateToProps = (state) => {
    // return {ui: state['uiReducer'], broker: state['brokersReducer'], stocks: state['stocksReducer']};
